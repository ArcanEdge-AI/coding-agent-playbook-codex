# Task Graph Orchestration Workflow
...
5. Read `task-graph-template.md` before creating a formal graph artifact.
6. Use `$worktree-lifecycle` when any node proposes or already owns an auxiliary worktree. Keep worktree permits separate from node permits.
...
Also reconcile every task-created auxiliary worktree before completion under `$worktree-lifecycle`, or preserve it with the exact path, owner, branch or HEAD, blocker, and next action. Do not leave task-owned cleanup to scheduled automation.
